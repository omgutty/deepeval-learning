"""DeepEval Dashboard - the control panel for Subsystem A's metric suite.

Runs the same metrics the pytest suite asserts, one card at a time, and shows
the score, the judge's reason and the latency for each. Built so a room full
of students can watch a metric pass or fail live.
"""
...
from targets.chatbot import CHATBOT_URL, ChatbotClient  # noqa: E402
...
def get_judge():
    """Built lazily: reading the judge's key should not block app startup."""
    global _judge
    if _judge is None:
        _judge = build_judge()
    return _judge
