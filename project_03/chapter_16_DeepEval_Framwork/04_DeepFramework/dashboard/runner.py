...
from targets.rag import RagClient
...
def prompt_of(item) -> str:
    ...
    if isinstance(item, str):
        return item
    return getattr(item, "input", None) or getattr(item, "prompt", None) or str(item)


def _ask_target(spec: MetricSpec, chatbot: ChatbotClient, text: str) -> str:
